...
class PlaNet:
    ...
    def _np_to_tensor(self, inputs_np: Tuple[ndarray], device:torch.device, dtype: torch.dtype) -> Tuple[Tensor]:
        return tuple(Tensor(x).to(device).to(dtype) for x in inputs_np)

    def __call__(self, measures: ndarray, coils_current:ndarray, profile: ndarray, rr: ndarray, zz: ndarray) -> ndarray:
